package com.epam.finaltask.university.controller.command;

/**
 * Created by Zheny Chaichits on 22.01.16.
 */
public enum CommandName {
    GO_HOME, GO_SIGNUP, GO_STATISTICS, GO_PROFILE, GO_SUPPORT_SEARCH, GO_APPLY_FORM, GO_APPLICATION,
    GO_APPLICATION_DATA, GO_USER_MANAGEMENT, GO_USER_DATA, GO_FACULTY_MANAGEMENT,

    CHECK_ACCOUNT, CHECK_EMAIL, CHECK_PASSPORT_ID,

    AUTHENTICATE, COOKIE_AUTHENTICATE, LOG_OUT, SIGN_UP,

    UPDATE_PROFILE, UPDATE_STUDENT_DATA, UPDATE_USER,

    CREATE_APPLICATION, CREATE_USER,
    DELETE_APPLICATION, DELETE_PROFILE, DELETE_USER,

    GET_SUBJECTS,
    SET_LOCALE,

    SEARCH_PROFILE_BY_PASSPORT, SEARCH_PROFILES_BY_LAST_NAME, SEARCH_APPLIED_BY_PASSPORT, SEARCH_APPLIED_BY_LAST_NAME,
    SEARCH_ALL_PROFILES, SEARCH_ALL_APPLIED, SEARCH_USERS_BY_ROLE,

    NO_SUCH_COMMAND;

    private static String CONTROLLER_NAME = "action";
    private static String COMMAND_QUERY_NAME = "?command=";

    public String getQueryString() {
        return CONTROLLER_NAME + COMMAND_QUERY_NAME + this.toString().toLowerCase();
    }
}
